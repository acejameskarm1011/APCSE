...
dataTO = pd.read_csv(filepath + "Cruise.csv", header=None).to_numpy()

attrTO = dataTO[0,1:].astype(str)

dataTO = dataTO[1:,1:].astype(float).T

timeArr, ThrustArr, LiftArr, DragArr, WeightArr, PercentArr, VelocityArr, AltArr, RangeArr, RPMArr, AOAArr = dataTO
...
ax[1,0].set_xlabel("Velocity [knots]")
ax[1,0].set_ylabel("AOA [deg]")



ax[1,1].plot(timeArr, RPMArr)
