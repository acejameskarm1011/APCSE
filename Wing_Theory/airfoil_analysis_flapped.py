...
f_upper = make_smoothing_spline(x_upper, y_upper, lam=0.01)
f_lower = make_smoothing_spline(x_lower, y_lower, lam=0.01)
...
def chamber_flapped(chord, Angle, x_arr):
    cleanChamber_func = lambda x: (f_upper(x) + f_lower(x))/2
    cleanChamber = cleanChamber_func(x_arr)
    flapPercent = (0.81)/(chord)
    origin = [1-flapPercent, cleanChamber_func(1-flapPercent)]
    Angle = -Angle*np.pi/180
    mag = 1/np.cos(Angle)
    transformation = mag*np.array([[np.cos(Angle), -np.sin(Angle)],
                                   [np.sin(Angle), np.cos(Angle)]])
    
    x_preRot = x_arr[x_arr > 1-flapPercent] - origin[0]
    z_preRot = cleanChamber[x_arr > 1-flapPercent] - origin[1]
    r_preRot = np.block([[x_preRot],
                         [z_preRot]])
    r_Rot = transformation@r_preRot
    x_Rot, z_Rot = r_Rot
    x_arr[x_arr >= 1-flapPercent] = x_Rot + origin[0]
    chamber = cleanChamber.copy()
    chamber[x_arr >= 1-flapPercent] = z_Rot + origin[1]
    plt.plot(x_arr, chamber)
    plt.grid()
    plt.ylim(-0.5,0.5)
    # plt.show()
...
def getCoefficients(chord, Angle, dtheta=np.pi*1e-2):
    theta = np.arange(0,np.pi + dtheta, dtheta)
    x_arr = 1/2*(1-np.cos(theta))
    chamberline = chamber_flapped(chord, Angle, x_arr)
    dzdx = [(chamberline[1]-chamberline[0])/(x_arr[1]-x_arr[0])]
    N = len(x_arr)
    for i in range(1,N-1):
        dzdx.append(1/2*(chamberline[i+1]-chamberline[i-1])/(x_arr[i+1]-x_arr[i-1]))
    dzdx.append((chamberline[-1]-chamberline[-2])/(x_arr[-1]-x_arr[-2]))
    df = pd.DataFrame({"dzdx":dzdx})
    df.to_csv("dzdx_flapped.csv")
    C_l_alpha = 2*np.pi
    A_1 = 2/(np.pi-dtheta)*np.trapz(dzdx*np.cos(theta), theta)
    C_l_0 = 2*np.pi*(- 1/(np.pi-dtheta)*np.trapz(dzdx, theta)) + np.pi*(A_1)
    print(C_l_0)
    return C_l_0, C_l_alpha
